...
import {
  ...
  setErrorCounter,
  setStepCount,
} from "../../../redux/slices/checkpointSlice";
...
const SortableListContainer: React.FC<SortableListContainerProps> = ({
  ...
  const challengeInfo = useSelector(
    (state: RootState) => state.checkpoint.challengeInfo
  );
  const completed = useSelector(
    (state: RootState) => state.checkpoint.completed
  );
  const errorCounter = useSelector(
    (state: RootState) => state.checkpoint.errorCounter
  );
  const activeStep = useSelector(
    (state: RootState) => state.checkpoint.activeStep
  );
  const challengeStatus = useSelector(
    (state: RootState) => state.checkpoint.challengeStatus
  );
  const stepCount = useSelector(
    (state: RootState) => state.checkpoint.stepCount
  );
  ...
  const goToNextStep = () => {
    ...
    if (
      Array.isArray(sortingSteps) &&
      sortingSteps[stepCount] &&
      sortingSteps[stepCount].array
    ) {
      if (
        arraysEqual(sortingSteps[stepCount].array, userSortingSteps[stepCount])
      ) {
        tempChallengeInfo.push({
          message: "correct",
          swap: sortingSteps[stepCount].swap,
        });
        dispatch(setChallengeInfo([...tempChallengeInfo]));
        dispatch(
          setCompleted(
            completed >= 100 ? 100 : completed + 100 / sortingSteps.length
          )
        );
      } else {
        tempChallengeInfo.push({
        ...
        dispatch(setChallengeInfo([...tempChallengeInfo]));
        dispatch(
          setErrorCounter(errorCounter >= 100 ? 100 : errorCounter + 33.3)
        );
      }
    }
  ...
  const goBack = () => {
    ...
    if (
      Array.isArray(sortingSteps) &&
      sortingSteps[stepCount - 2] &&
      sortingSteps[stepCount - 2].array
    ) {
  ...
  return (
    <>
      ...
      <br />
      <Button variant="outlined" size="small" onClick={goToNextStep}>
        NEXT
      </Button>
      &nbsp;&nbsp;
